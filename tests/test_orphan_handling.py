...
def test_orphan_elements_are_mapped_to_correct_buildings():
    ...
    # Build model tree
    element_ids = [1, 2, 3, 4]
    model_trees = service.build_model_element_tree_nodes(element_ids)
    ...
    assert len(
        building1_nodes) == 2, f"Building1 should have 2 nodes (wall + orphan wrapper), got {len(building1_nodes)}"
...
def test_orphan_parent_does_not_generate_invalid_element_ids():
    ...
    model_trees = service.build_model_element_tree_nodes([1])
